export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '对 OpenAI OAuth Responses 会话，将指纹层拥有的客户端标识收敛为账号级恒定值；默认为关闭，收敛需显式开启。Native Compact v2 遵循所选模式；ChatGPT Codex OAuth 的 legacy compact 兼容路径仅使用稳定 installation 标识。关闭只停用指纹改写，不会停用 Plus 缓存、安全或会话策略。',
        codexFingerprintOff: '关闭（默认，不改写指纹）',
        codexFingerprintDevice: '仅设备',
        codexFingerprintSession: '设备+会话',
        codexFingerprintFull: '完全收敛',
        codexFingerprintSeedPending: '该存量账号尚无指纹种子，当前收敛未生效；保存后将生成种子并开始收敛（一次性设备身份轮换）。',
